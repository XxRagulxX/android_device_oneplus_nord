...
# Repo URLs and branch options (add -b "branchname" if needed)
FW_REPO="https://github.com/XxRagulxX/android_firmware_oneplus_avicii"
KERNEL_REPO="https://github.com/XxRagulxX/android_kernel_oneplus_sm7250" -b "KernelSU-Next"
...
# Dependencies: name, directory, repo variable
declare -a DEPENDENCIES=(
  "FW|${FW_DIR}|FW_REPO"
  "KERNEL|${KERNEL_DIR}|KERNEL_REPO"
...
chk_dependencies() {
  ...
  for dep in "${DEPENDENCIES[@]}"; do
    IFS='|' read -r NAME DIR REPO_VAR <<< "$dep"
    REPO_CMD="${!REPO_VAR}"
    if [[ -d "${DIR}" ]]; then
      ...
      git clone --depth=1 ${REPO_CMD} "${DIR}"
...
install_kernelsu_next() {
  ...
  LATEST_URL=$(curl -s https://api.github.com/repos/KernelSU-Next/KernelSU/releases/latest | \
